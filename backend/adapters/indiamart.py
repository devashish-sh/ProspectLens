# backend/adapters/indiamart.py
# ProspectLens — IndiaMART Extraction Adapter
...
class IndiaMartAdapter(BaseAdapter):
    ...
    def get_site_information(self) -> dict:
        return {
            ...
            "adapter_class": "IndiaMartAdapter"
        }
    ...
    def is_search_results_page(self, url: str) -> bool:
        url = url.lower()
        return "search.mp" in url or "/impcat/" in url or "/messages/" in url or "dir.indiamart.com" in url

    def is_detail_page(self, url: str) -> bool:
        url = url.lower()
        return "/proddetail/" in url or "/company/" in url
    ...
    def get_page_type(self, url: str) -> str:
        ...
        if "/login" in url or "signin" in url:
            return "Login"
        if "/myprofile" in url or "/seller/" in url:
            return "Profile"
        if url.endswith("indiamart.com") or url.endswith("indiamart.com/"):
            return "Home"
        return "Unknown"
