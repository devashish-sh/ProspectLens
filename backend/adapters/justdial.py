# backend/adapters/justdial.py
# ProspectLens — Justdial Extraction Adapter

from adapters.base import BaseAdapter

class JustdialAdapter(BaseAdapter):
    def validate_page(self, url: str) -> bool:
        return "justdial.com" in url.lower()
    ...
    def is_search_results_page(self, url: str) -> bool:
        url = url.lower()
        # Search index pages typically do not end with dht or detail pattern
        return ("justdial.com/" in url) and not self.is_detail_page(url) and not self.is_home_or_auth(url)

    def is_detail_page(self, url: str) -> bool:
        url = url.lower()
        return "/detail/" in url or url.endswith("dht") or "/dht" in url

    def is_supported_page(self, url: str) -> bool:
        return self.is_search_results_page(url) or self.is_detail_page(url)

    def is_home_or_auth(self, url: str) -> bool:
        url = url.lower().strip().rstrip("/")
        return url.endswith("justdial.com") or "/login" in url or "/signin" in url
    ...
    def get_page_type(self, url: str) -> str:
        ...
        if "/login" in url or "signin" in url:
            return "Login"
        if url.endswith("justdial.com") or url.endswith("justdial.com/"):
            return "Home"
        return "Unknown"
